...
简书：<https://www.jianshu.com/p/812636d55677>
...
- 用户界面
  负责绘制用户用来和应用交互的屏幕界面并将用户的输入翻译成应用的命令。值得注意的是“用户”可以是人类也可以是连接我们 API 的其他应用，它们和[EBI 架构](https://herbertograca.com/2017/08/24/ebi-architecture/)中的边界对象完全对应。
- 应用层
  协调领域对象完成用户要求的任务：用例。它不包含业务逻辑。应用层和[EBI 架构](https://herbertograca.com/2017/08/24/ebi-architecture/)中的交互器相对应，只有一点不同，交互器是和界面或实体无关的任意对象，而这里应用层只包含和用例相关的对象。应用服务属于这一层，它们是用例对资源库、领域模型、实体、值对象或是任何其它领域对象进行编配的容器。
- 领域层
  这个层次包含了所有的业务逻辑，如领域服务、实体、事件和其他包含业务逻辑的任意对象类型。显然它和 EBI 架构中的实体对象类型对应。这是系统的心脏。领域服务包含的领域逻辑不太适合放到某个实体中，通常是为了完成某个领域操作而对多个实体进行的编配。
- 基础设施
